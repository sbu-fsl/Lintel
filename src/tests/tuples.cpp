...
int main() {
    ...
    INVARIANT(lintel::hash(tuple1) == 2462433311U, format("got %d?") % lintel::hash(tuple1));
    INVARIANT(lintel::hash(tuple2) == 1460156332U, format("got %d?") % lintel::hash(tuple2));
    ...
    INVARIANT(lintel::hash(anytuple1) == 2462433311U, format("%d?") % lintel::hash(anytuple1));
    INVARIANT(lintel::hash(anytuple2) == 1460156332U, format("%d?") % lintel::hash(anytuple2));
    ...
    SINVARIANT(anytuple1 < anytuple2);
    INVARIANT(lintel::hash(anytuple1) == 2138559752U, format("%d?") % lintel::hash(anytuple1));
    INVARIANT(lintel::hash(anytuple2) == 1910363323U, format("%d?") % lintel::hash(anytuple2));

    anytuple1.get<1>().any = true;
    anytuple2.get<1>().any = true;
    SINVARIANT(anytuple2 < anytuple1);

    INVARIANT(lintel::hash(anytuple1) == 1248248738U, format("%d?") % lintel::hash(anytuple1));
    INVARIANT(lintel::hash(anytuple2) == 1959181482U, format("%d?") % lintel::hash(anytuple2));
